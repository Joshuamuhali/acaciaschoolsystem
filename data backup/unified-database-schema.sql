...
-- Drop existing parent tables if they exist (for clean reset)
DROP TABLE IF EXISTS pupil_parent_relationships CASCADE;
DROP TABLE IF EXISTS parents CASCADE;
DROP SEQUENCE IF EXISTS parent_code_seq CASCADE;
DROP FUNCTION IF EXISTS generate_parent_code() CASCADE;
...
-- Pupil-Parent relationship table
...
-- Terms (with term number and months count)
CREATE TABLE terms (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(100) NOT NULL,
...
-- Families (group pupils by parent)
...
-- Other fees (lunch, transport, maintenance, registration, sports, library, PTC)
...
BEGIN
    ...
    -- 5. Maintenance fee (always, per term)
    INSERT INTO other_fees (pupil_id, term_id, fee_type, total_expected, collected, balance)
    VALUES (NEW.id, current_term.id, 'Maintenance', 250, 0, 250);

    -- 6. Lunch fee (if pupil takes lunch)
    IF NEW.takes_lunch THEN
        INSERT INTO other_fees (pupil_id, term_id, fee_type, total_expected, collected, balance)
        VALUES (NEW.id, current_term.id, 'Lunch', 1200, 0, 1200);
    END IF;

    -- 7. Transport fee (if route assigned)
    IF NEW.transport_route_id IS NOT NULL THEN
        ...
        INSERT INTO other_fees (pupil_id, term_id, fee_type, total_expected, collected, balance)
        VALUES (NEW.id, current_term.id, 'Transport', v_transport_total, 0, v_transport_total);
    END IF;

    -- 8. Registration fee (one-off for new pupils)
    IF NEW.status = 'new' THEN
        ...
        IF NOT v_registration_exists THEN
            INSERT INTO other_fees (pupil_id, term_id, fee_type, total_expected, collected, balance)
            VALUES (NEW.id, current_term.id, 'Registration', 200, 0, 200);
        END IF;
    END IF;

    -- 9. Sports fund (only Term 1)
    IF current_term.term_number = 1 THEN
        INSERT INTO other_fees (pupil_id, term_id, fee_type, total_expected, collected, balance)
        VALUES (NEW.id, current_term.id, 'Sports', 200, 0, 200);
    END IF;

    -- 10. Library fee (only Term 1 and Grades 1–7)
    IF current_term.term_number = 1 THEN
        ...
        IF EXISTS (
            ...
            INSERT INTO other_fees (pupil_id, term_id, fee_type, total_expected, collected, balance)
            VALUES (NEW.id, current_term.id, 'Library', 270, 0, 270);
        END IF;
    END IF;

    -- 11. PTC fee (only once per family per term)
    SELECT EXISTS (
...
-- =====================================================
-- COMPLETE PARENT SYSTEM RESET
-- WARNING: This deletes all existing parent data
-- =====================================================

-- Drop relationship table first (because of FK dependency)
DROP TABLE IF EXISTS pupil_parent_relationships CASCADE;
...
-- =====================================================
-- 1. CREATE SEQUENCE FOR PARENT CODE
-- =====================================================
...
-- =====================================================
-- 2. FUNCTION TO GENERATE PARENT CODE (ACL00001)
-- =====================================================
...
-- =====================================================
-- 7. PUPIL-PARENT RELATIONSHIP TABLE
-- =====================================================
...
-- =====================================================
-- 8. ENFORCE ONE PRIMARY CONTACT PER PUPIL
-- =====================================================
